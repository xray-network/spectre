...
Canonical-URL: https://wiki.xraynetwork.io/spectre/protocol/v2.0.0/SPECTRE-PROTOCOL.md
...
SPECTRE commands are concise selectors for existing operations. They do not create a parallel
workflow or grant authority beyond the operation they select:

| Syntax | Operation and stopping boundary |
| --- | --- |
| `SPECTRE HELP` | Read this command vocabulary and report every canonical command with its supported syntax, purpose, and stopping boundary without changing tracked files or lifecycle state. |
| `SPECTRE HELP <command>` | Report the supported syntax, purpose, and stopping boundary for one canonical command without changing tracked files or lifecycle state. |
...
| `SPECTRE REVISE <target>/<NNNN>: <requested changes>` | Run the §9 revision workflow for the identified `REVIEW` record, change only that implementation within its instruction, rerun applicable validation, update its existing result, keep it in `REVIEW`, and stop. |
| `SPECTRE LIST` | Read the aggregate status ledger and list every target and implementation in every state. |
| `SPECTRE LIST <target>` | Read the aggregate status ledger and list every implementation for one target. |
| `SPECTRE LIST <target> <state>` | Read the aggregate status ledger and list one target's implementations filtered to the named lifecycle state. |
...
| `SPECTRE VALIDATE` | Run applicable §13 validation for the installed SPECTRE structure and report outcomes without changing tracked files or lifecycle state. |
| `SPECTRE VALIDATE <target>/<NNNN>` | Run applicable instruction and §13 validation for one record and report outcomes without changing tracked files or lifecycle state. |
| `SPECTRE ACCEPT <target>/<NNNN>: <decision proof>` | Record the current human's acceptance of a `REVIEW` record and only the matching ledger decision fields. |
| `SPECTRE REJECT <target>/<NNNN>: <decision proof>` | Record the current human's rejection of a `REVIEW` record and only the matching ledger decision fields. |
...
The `SPECTRE` prefix, command word, command-name argument to `HELP`, and lifecycle-state filter are
...
For example, `SPECTRE LIST api PLANNED`, `spectre list api planned`, and
`SpEcTrE LiSt api pLaNnEd` select the same query. Likewise, `SPECTRE IMPLEMENT api/0002` and
`spectre implement api/0002` select the same implementation operation, while
`SPECTRE REVISE homepage/0002: adjust spacing` and
`spectre revise homepage/0002: adjust spacing` select the same revision operation, while
`SPECTRE HELP IMPLEMENT` and `spectre help implement` select the same command help. Other arguments
...
> Read `.spectre/SPECTRE-PROTOCOL.md` completely and install SPECTRE v2.0.0 in this repository.
...
3. Create `.spectre/`, `.spectre/templates/` with the three templates in §11, one aggregate
   `SPECTRE.md` in the repository root, and the flat or monorepo implementation layout
   selected by §4. Never create a product target directory merely because a target name can be
   inferred.
4. Create `.spectre/README.md` from §10.
...
- Read `.spectre/SPECTRE-PROTOCOL.md` before planning, implementing, or revising tracked changes.
...
4. This `.spectre/SPECTRE-PROTOCOL.md` standard.
5. The templates under `.spectre/templates/`.
...
### `.spectre/templates/TEMPLATE_STATUS.md`
...
Protocol: [.spectre/SPECTRE-PROTOCOL.md](.spectre/SPECTRE-PROTOCOL.md)
...
| --- | --- | --- | --- | --- | --- | --- |
| `0001` | Install SPECTRE | [Instruction](.spectre/implementations/0001-IMPL-INSTR.md) | `ACCEPTED` | [Result](.spectre/implementations/0001-IMPL-RESULT.md) | `LOCAL` | Human requested installation of SPECTRE. |
```
...
under `.spectre/implementations/<target>/`. Every section's table header is required even when there are
...
### `.spectre/templates/TEMPLATE_IMPL.md`
...
### `.spectre/templates/TEMPLATE_PROVIDER.md`
...
- The three canonical templates exist only under `.spectre/templates/`.
...
- Single-project repositories store implementation records directly under
  `.spectre/implementations/`; monorepos store them only under target directories.
- Flat and nested implementation records do not coexist unless a human-approved structural
  migration is in progress.
- No target-local `STATUS.md` exists below `.spectre/implementations/`.
...
`.spectre/SPECTRE-PROTOCOL.md`, not by a mutable remote page.
...
4. Replace `.spectre/SPECTRE-PROTOCOL.md`, then update non-terminal templates and structure as
   required.
...
4. Remove `.spectre/` only after a human confirms the exact path.
...
```text
download .spectre/SPECTRE-PROTOCOL.md
        ↓
